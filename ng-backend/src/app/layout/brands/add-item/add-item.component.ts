...
import { ICakeCategory, IFlavour, IShop } from '../../../models/product.items';
...
export class AddBrandsItemComponent {
  error = false;
  ...
  async saveData() {
    ...
    if (this.productForm.valid) {
      try {
        ...
        const result = await this.fireService.saveFlavourItem(data);
        console.log(result);
        LoadingHelper.hide();
        this.router.navigateByUrl('/app/brands');
      } catch (error) {
        console.error(error);
      }
    } else {
